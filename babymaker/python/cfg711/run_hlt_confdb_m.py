# /users/jaehyeok/HLTRun2/RA4/V5 (CMSSW_7_1_6)

import FWCore.ParameterSet.Config as cms

process = cms.Process( "reHLT" )
process.load("HLTStudy.HLTBabyMaker.setup_cff")
#process.load("HLTStudy.HLTBabyMaker.babyMakerE_cfi")
process.load("HLTStudy.HLTBabyMaker.babyMakerM_cfi")

process.HLTConfigVersion = cms.PSet(
  tableName = cms.string('/users/jaehyeok/HLTRun2/RA4/V5')
)
...
#process.HLT_Ele15_WP80_Gsf_PFHT200_PFMET0_v1 = cms.Path( process.HLTBeginSequence + process.hltL1sL1HTT150OrHTT175 + process.hltPreEle15WP80GsfPFHT200PFMET0 + process.HLTAK4CaloJetsSequence + process.hltHtMht + process.hltHt100 + process.HLTAK4PFJetsSequence + process.hltPFHT + process.hltPFHT200 + process.hltPFMETProducer + process.hltPFMET0Filter + process.hltL1sL1SingleEG20ORL1SingleEG22 + process.hltPreEle27WP80Gsf + process.HLTEle27WP80GsfSequence + process.antiktGenJets + process.HLTEndSequence + process.babyMakerE )
process.HLT_Mu15_PFHT200_PFMET0_v1 = cms.Path( process.HLTBeginSequence + process.hltL1sL1HTT150OrHTT175 + process.hltPreEle15WP80GsfPFHT200PFMET0 + process.HLTAK4CaloJetsSequence + process.hltHtMht + process.hltHt100 + process.HLTAK4PFJetsSequence + process.hltPFHT + process.hltPFHT200 + process.hltPFMETProducer + process.hltPFMET0Filter + process.hltL1sMu16 + process.hltPreMu40 + process.hltL1fL1sMu16L1Filtered0 + process.HLTL2muonrecoSequence + process.hltL2fL1sMu16L1f0L2Filtered0Q + process.HLTL3muonrecoSequence + process.hltL3fL1sMu16L1f0L2f16QL3Filtered15Q + process.antiktGenJets + process.HLTEndSequence + process.babyMakerM )
...
process.out = cms.OutputModule(
        "PoolOutputModule",
        fileName     = cms.untracked.string('ntuple_hlt_m.root'), ##
#        fileName     = cms.untracked.string('edm_hlt_e.root'),
...
#process.out.outputCommands.extend(cms.untracked.vstring('keep *_*hltAK4PFJetL1FastL2L3Corrected*_*_*'))
#process.out.outputCommands.extend(cms.untracked.vstring('keep *_*hltPFHT*_*_*'))
#process.out.outputCommands.extend(cms.untracked.vstring('keep *_*hltPFMETProducer*_*_*'))
#process.out.outputCommands.extend(cms.untracked.vstring('keep *_*hltRHemisphere*_*_*'))
#process.out.outputCommands.extend(cms.untracked.vstring('keep *_*babyMakerE*_*_*')) ##
process.out.outputCommands.extend(cms.untracked.vstring('keep *_*babyMakerM*_*_*')) ##
