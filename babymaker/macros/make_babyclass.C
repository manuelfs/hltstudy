...
//-------------------------------------------------------------------------------------------------
void make_babyclass (const std::string& fname, bool paranoid = true, const std::string& treeName="Events",
		     const std::string& branchNamesFile="", const std::string& className="hlt_class",
		     const std::string& nameSpace="baby", const std::string& objName="hlt") {
  ...
  gErrorIgnoreLevel=kError; // Turns off "no dictionary for class" warnings
    ...
    f->Close();
    cout<<endl<<"Written "<<className<<".h and "<<className<<".cc"<<endl<<endl;
}
